...
tags: [python, testing, xslt, reporting]
description: "The default HTML test report generator for the Python nosetests library produces output that is functional but barely readable — dense, unstyled, and difficult to navigate as a test suite grows. This project is an XSLT plugin that transforms Xunit/JUnit XML reports into clean, modern HTML with clear pass/fail summaries, expandable test details, and sensible visual hierarchy. It works with any tool that emits standard Xunit-formatted XML, not just nosetests. The plugin is available on GitHub and can be dropped into any CI pipeline that already generates XML test output."
