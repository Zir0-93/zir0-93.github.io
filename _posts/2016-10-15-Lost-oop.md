...
Interestingly enough, many of the key concepts Alan envisioned as being part of the definition of OOP are not found in most
...
Objects are commonly thought as central to the understanding of OOP. Surely it is  called **Object** oriented programming for a reason.
...
## The Role of Objects
One of the main principles in object oriented systems today model objects after living things. And yet, most object oriented
systems fail miserably at this goal in many ways. Consider the example below taken from a tutorial on encapsulation from
tutorialspoint.com. This example is thought to demonstrate encapsulation because the private field variables are not accessible
...
Alan's objects would be self contained and hide internal data properly. Alan's visions of objects also clashes with inheritance
as the evil ```extends``` word in Java for example results in fragile objects that are highly coupled to their inheritance hierarchy.
...
are associated with OOP today would not exist had his understanding of OOP been taken seriously. Indeed,
many of the promised advantages of OOP including encapsulation and separation of concerns are not actually realized in the current way
OOP is commonly practiced. Rather,
by implementing the ideas behind messaging combined with the creation of self-contained, immutable objects, our object oriented systems
actually begin to demonstrate the information hiding mechanism promised by encapsulation while being growable at the same time. 
